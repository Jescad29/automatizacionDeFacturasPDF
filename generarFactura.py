...
def generar_factura():
    ...
    rfc_cliente = RFC_cliente_entry.get()
    ...
    total_servicio = total_cliente_entry.get()
    ...
    pdf = FPDF()
    pdf.add_page()
    logo_path = 'hubcode.jpg'
    pdf.image(logo_path, x=160, y=8, w=40)
    
    pdf.set_xy(10, 10)
    pdf.set_font('Arial', size=12)
    pdf.cell(0, 10, 'HubCode', ln=True)
    pdf.cell(0, 10, 'Queretaro, Queretaro CP: 07539 Alameda', ln=True)
    pdf.cell(0, 10, 'CEC63743723C32', ln=True)
    ...
    pdf.cell(0, 10, f'RFC: {rfc_cliente}', ln=True, align='L')
    ...
    pdf.cell(60,10, 'Descripción del servicio', border=1)
    pdf.cell(60,10, 'Total', border=1, ln=True)
    
    pdf.cell(60,10, nombre_servicio, border=1)
    pdf.cell(60,10, descripcion_servicio, border=1)
    pdf.cell(60,10, total_servicio, border=1, ln=True)
    
    pdf.cell(0,10, '===================================================================================================', ln=True, align='L')
    pdf.cell(0,10, 'Gracias por su compra!', ln=True, align='C')
    
    pdf_file = f'Factura {nombre_cliente}_{apellidos_cliente}.pdf'
    pdf.output(pdf_file,'F')
    messagebox.showinfo('Facturado Generada')
...
ttk.Label(frame, text = 'RFC del Cliente').grid(row=3, column=0, sticky=tk.W)
RFC_cliente_entry = ttk.Entry(frame, width=30)
RFC_cliente_entry.grid(row=3, column=1, sticky=(tk.W, tk.E))
...
total_cliente_entry = ttk.Entry(frame, width=30)
total_cliente_entry.grid(row=7, column=1, sticky=(tk.W, tk.E))
